...
@Suspendable
public class DynamicFileServerFiber implements Runnable {
    ...
    public DynamicFileServerFiber(Map.Entry<String,String> rd, RequestParser rp, byte[] h, int len, Socket socket) {
        l = len;
        this.h = h;
        requestParser = rp;
        this.rd = rd;
    ...
    public void run() {
        try {
            _sock.shutdownInput();
            System.out.println("Running File server");
            ...
            if (extPath.length()>1 && (!fp.toFile().isDirectory())){
                ...
                if (Files.exists(fp) && (!(npath.toString().contains("../")||(npath.toString().contains("/.."))))) {
                    String key = con.getOption("key");
                    String reskey = prts[prts.length-1]+"_"+key;
                    if (jd.exists(reskey)) {
                        System.out.println(key+" is Member of File : "+prts[prts.length-1]);
                        long rk = Long.valueOf(jd.get(reskey));
                        if (rk<=0) {
                            //jd.srem(prts[prts.length-1], key);
                            jd.del(reskey);
                            jd.close();
                            con.writer.getHeader().setStatus(ResponseCode.FORBIDDEN);
                            con.writer.writeAll("Invalid Key!\r\n");
                            con.writer.finish();
                            return;
                        } else {
                            jd.decr(reskey);
